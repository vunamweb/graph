...
import com.example.administrator.myapplication.models.GraphPoint;
...
public class CurveGraphView extends View {
    ...
    private List<Paint> listPain = new ArrayList<Paint>();
    ...
    private void drawGraphPoints(Canvas canvas) {
        for (int i = 0; i < graphPointsList.size(); i++) {
            for (GraphPoint gp : graphPointsList.get(i)) {
                ...
                //drawtext vunam
                //float y = ((graphHeight - 32) * gp.getY() / totalY()) + graphPadding + yAxisScalePaint.getTextSize() / 2;
                float x = gp.getX();
                /*if(graphPointsList.size() > 1)
                  x = (graphWidth * i) / (graphPointsList.size() - 1);*/

                float y = gp.getY();
                /*if(graphPointsList.size() > 1)
                  y = (graphHeight / totalY()) * (totalY() - gp.getY());*/
   ...
   /* private float totalY() {
    ...
    private ArrayList<Path> constructPaths() {
        ...
        for (int i = 0; i < graphDataArray.length; i++) {
            ...
            GraphPoint prevDataPoint = new GraphPoint(graphPadding, morphedGraphHeight, null);
            Path path = new Path();
    ...
    private void drawScaleText(Canvas canvas, GraphData[] graphData) {
        if (maxVal > 0) {
            for (float i = 5; i > 0; i--) {
                float value = maxVal * i / 5;
                ...
                float y = ((graphHeight - 32) * (5 - i) / 5f) + graphPadding + yAxisScalePaint.getTextSize() / 2;
                //canvas.drawText(String.valueOf((int) value), graphWidth - graphPadding + 8, y, yAxisScalePaint);
                canvas.drawText("nam", graphWidth - graphPadding + 8, y, yAxisScalePaint);
            }
        } else {
            canvas.drawText(this.noDataMsg, (graphWidth - graphPadding * 2) / 2, (graphHeight + graphPadding * 2) / 2, yAxisScalePaint);
        }
